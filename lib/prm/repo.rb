...
module Debian
    ...
    def generate_release_gpg(path,release,gpg)
        Dir.chdir("#{path}/dists/#{release}") do
            ...
            if gpg.nil?
              sign_cmd = "gpg  --armor --digest-algo \"#{sign_algorithm}\" --no-tty --yes --output Release.gpg -b Release"
            elsif gpg_passphrase
              sign_cmd = "echo \'#{gpg_passphrase}\' | gpg  --armor --digest-algo \"#{sign_algorithm}\" --no-tty -u #{gpg} --passphrase-fd 0 --yes --output Release.gpg -b Release"
            else
              sign_cmd = "gpg  --armor --digest-algo \"#{sign_algorithm}\" -u #{gpg} --no-tty --yes --output Release.gpg -b Release"
            end
            system sign_cmd
        end
    end
